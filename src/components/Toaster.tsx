      ...
      <AnimatePresence>
        {toasts.map((t) => (
          <motion.div
            ...
            className={`pointer-events-auto rounded-2xl border p-3 flex items-start gap-3 bg-panel text-ink shadow-lg backdrop-blur-2xl ${COLORS[t.kind]}`}
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-black text-ink">{t.title}</div>
              {t.body && <div className="text-xs text-mute mt-0.5 leading-snug">{t.body}</div>}
            </div>
            <button
