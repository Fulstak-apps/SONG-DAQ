...
export function RiskBadge({ coin, compact = false }: { coin: Partial<AudiusCoin> & Record<string, any>; compact?: boolean }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-md border px-2 py-1 text-[8px] font-black uppercase tracking-widest ${cls}`} title={risk.warnings.join(" ") || "Risk checks passed"}>
  ...
  return (
    <section className="panel p-5 shadow-xl space-y-4">
      <div className="flex items-center justify-between gap-3 border-b border-edge pb-3">
        <div>
          <div className="text-[10px] uppercase tracking-widest font-black text-mute">Market Safety</div>
      ...
      <div className="grid gap-2">
        {rows.map(([k, v]) => (
          <div key={k} className="flex items-center justify-between gap-3 text-xs">
            <span className="text-mute uppercase tracking-widest font-bold text-[10px]">{k}</span>
