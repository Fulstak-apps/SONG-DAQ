...
export function SongCard({ s, compact = false }: { s: SongRow; compact?: boolean }) {
  ...
  const playerTrack = useMemo<PlayerTrack | null>(() => s.streamUrl ? ({
    id: String(s.id),
    title: s.title,
    artist: s.artistName,
    artwork: s.artworkUrl ?? s.coverUrl ?? null,
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl overflow-hidden shrink-0 border border-edge shadow-lg relative bg-panel2">
            {s.coverUrl ? (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="relative w-14 h-14 rounded-xl overflow-hidden border border-edge bg-panel2 shadow-depth shrink-0">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            ...
            className={`w-7 h-7 flex items-center justify-center rounded-lg transition-all ${
              isWatched ? "bg-gold/10 text-gold border border-gold/20" : "text-mute hover:text-ink"
            }`}
          >
...
export function SongListRow({ s }: { s: SongRow }) {
  ...
  return (
    <motion.div
      whileHover={{ x: 4 }}
      onClick={() => router.push(`/song/${s.id}`)}
      className="panel p-3 flex items-center gap-4 cursor-pointer hover:bg-white/[0.07] transition-all duration-300 group"
    >
      <button
        onClick={(e) => { e.stopPropagation(); watchlist.toggle(s.id); }}
        className={`w-6 h-6 flex items-center justify-center rounded-md shrink-0 transition-all ${
          watchlist.items.includes(s.id) ? "text-gold" : "text-mute hover:text-ink"
        }`}
      >
